...
import { postQueryOptions } from '@/entities/post/model/queries/useGetPosts';
import type { GetPostsItemResponseDto } from '@/entities/post/model/types';
...
const getPostPreviewText = (content: string) => {
  const withoutTags = content.replace(HTML_TAG_PATTERN, ' ');
  const normalized = withoutTags
    .replace(HTML_ENTITY_PATTERN, decodeHtmlEntity)
    .replace(/\s+/g, ' ')
    .trim();

  return normalized || COPY.htmlOnlyPost;
};
...
const createRecentSearches = (keyword: string, recentSearches: string[]) => {
  const trimmedKeyword = keyword.trim();
...
const formatEmptyResultMessage = (keyword: string) =>
  COPY.noSearchResult.replace('%s', keyword);

const mapPostToCard = (post: GetPostsItemResponseDto): FeedSearchPost => ({
  id: post.postId,
  author: post.writerNickname,
  createdAt: post.createdAt,
  avatarUrl: post.writerProfileImageUrl ?? undefined,
  content: getPostPreviewText(post.content),
          ...
          <section className="tablet:pt-[120px] flex items-center justify-center px-4 pt-[160px] text-center">
            <div className="flex w-[176px] flex-col items-center gap-8">
              <p className="typo-body-16-regular whitespace-pre-line text-gray-500">
                {formatEmptyResultMessage(trimmedDeferredKeyword)}
              </p>
