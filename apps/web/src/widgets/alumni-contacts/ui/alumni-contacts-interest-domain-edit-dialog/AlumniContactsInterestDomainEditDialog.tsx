'use client';

import { TextInput } from '@causw/cds';

import { AlumniContactsFieldEditButton } from '@/features/alumni-contacts';

import { ALUMNI_CONTACTS_EDIT_FORM_FIELD } from '@/entities/alumni-contacts';

import { useAlumniContactsSingleFieldEditDialog } from '../../model';
import { AlumniContactsSingleFieldEditDialog } from '../alumni-contacts-single-field-edit-dialog';

interface AlumniContactsInterestDomainEditDialogProps {
  interestDomainIndex: number;
}

export const AlumniContactsInterestDomainEditDialog = ({
  interestDomainIndex,
}: AlumniContactsInterestDomainEditDialogProps) => {
  const {
    isOpen,
    currentFieldValue: currentInterestDomain,
    saveButtonRef,
    handleInitialFocus,
    handleClickDialogTrigger,
    handleOpenChange,
    handleFieldValueChange: handleCurrentInterestDomainChange,
    handleCompositionStart,
    handleCompositionEnd,
    handleEnterPress,
    handleClickSaveButton,
    handleClickDeleteButton,
  } = useAlumniContactsSingleFieldEditDialog({
    fieldName: ALUMNI_CONTACTS_EDIT_FORM_FIELD.USER_INTEREST_DOMAIN,
    fieldItemIndex: interestDomainIndex,
  });

  return (
    <>
      <AlumniContactsFieldEditButton
        iconSize={12}
        iconColor="gray-600"
        onClick={handleClickDialogTrigger}
        ariaLabel="관심 도메인 수정하기"
      />
      <AlumniContactsSingleFieldEditDialog
        isOpen={isOpen}
        onOpenChange={handleOpenChange}
        title="관심 도메인 수정"
        ariaDescription="관심 도메인을 수정합니다."
        canSave={!!currentInterestDomain.trim()}
        onSave={handleClickSaveButton}
        saveButtonRef={saveButtonRef}
        deleteButtonLabel="관심 도메인 삭제"
        onDelete={handleClickDeleteButton}
      >
        <TextInput
          placeholder="관심 도메인을 입력해주세요."
          className="bg-gray-100"
          onChange={handleCurrentInterestDomainChange}
          onCompositionStart={handleCompositionStart}
          onCompositionEnd={handleCompositionEnd}
          onKeyDown={handleEnterPress}
          value={currentInterestDomain}
          ref={handleInitialFocus}
        />
      </AlumniContactsSingleFieldEditDialog>
    </>
  );
};
