...
interface LandingAlumniConnectionCardProps {
  image: string;
  imageAlt: string;
  title: string;
  description: string;
...
export const LandingAlumniConnectionCard = ({
  image,
  imageAlt,
  title,
  description,
}: LandingAlumniConnectionCardProps) => {
  return (
    <div className="tablet:flex-row tablet:items-center tablet:gap-8 flex w-full flex-col gap-4">
      <div className="tablet:flex-1 relative aspect-104/89 w-full shrink-0 overflow-hidden rounded-2xl">
        <Image
          src={image}
          alt={imageAlt}
          fill
      ...
      <VStack className="tablet:flex-1 w-full" gap="xs">
        ...
        <Text
          ...
          textColor="gray-500"
          className="desktop:text-xl! whitespace-pre"
        >
          {description}
        </Text>
      </VStack>
